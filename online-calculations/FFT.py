...
cdm = PyCDM("172.16.7.210")
dq = cdm.reg_msg("DATA.FRAME.L1")

def empty_queue(dq,buffer, chanels = range(20)):
    while (True):
        try:
            data = dq.get(block=True, timeout=0.01)[1]
        except queue.Empty:
            #print("Empty message")
            break
...
def save_file(result, chanels=range(20)):
    for i in chanels:
        result[i].to_csv('./Output/'+time.strftime('%Y%m%d%H%M%S',time.gmtime())+f'-fft-{i}.csv')
    

def signal_handler(signum, frame):
    signal.signal(signum, signal.SIG_IGN)
    print('Captured Ctrl-C, saving data...')
    save_file(result) 
    cdm.close()
    sys.exit(0)
...
while True:
    #try:
        ...
        if first and len(buffer[0])== 2048:
            ...
            freq = np.fft.rfftfreq(2048,timestep/1e3)
            refactor_time = time.time() - buffer[20][-1]/1e3
            result = computefft(buffer, freq, result)#, chanels = [0,1])
            first= False
            #print(result)
            #print('Computed fft')
        elif not first and time.time()-result[0]['timestamp'].iloc[-1] > 5 :
            ts = [refactor_time + x/1e3 for x in buffer[20]]
